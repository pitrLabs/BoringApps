...
import com.pitrlabs.boringapps.CalculateHeatCapacityQuery
...
@Composable
fun HeatCapacityScreen(hazeState: HazeState) {
    ...
    val deltaTValue = deltaT.toDoubleOrNull()
            ...
            Box(
                ...
                Column(
                    ...
                    Button(
                            ...
                            scope.launch {
                                ...
                                try {
                                    val response = ApolloClientInstance.client
                                        .query(
                                            CalculateHeatCapacityQuery(
                                                m = mValue ?: 0.0,
                                                c = cValue ?: 0.0,
                                                deltaT = deltaTValue ?: 0.0
                                            )
                                        )
                                        .execute()
                                    result = response.data?.calculateHeatCapacity
                                } catch (e: Exception) {
                                    result = null
                                } finally {
                                    isLoading = false
                        ...
                        enabled = m.isNotBlank() && c.isNotBlank() && deltaT.isNotBlank()
                    ) {
                        Text("Calculate")
                    }
