...
import com.pitrlabs.boringapps.CalculateNernstQuery
...
@Composable
fun NernstScreen(hazeState: HazeState) {
    var n by remember { mutableStateOf("") }
    var eo by remember { mutableStateOf("") }
    var q by remember { mutableStateOf("") }
    var result by remember { mutableStateOf<Double?>(null) }
    var isLoading by remember { mutableStateOf(false) }

    val scope = rememberCoroutineScope()
    val nValue = n.toDoubleOrNull()
    val eoValue = eo.toDoubleOrNull()
    val qValue = q.toDoubleOrNull()
            ...
            Box(
                ...
                Column(
                    ...
                    Button(
                            ...
                            scope.launch {
                                ...
                                try {
                                    val response = ApolloClientInstance.client
                                        ...
                                        .execute()
                                    result = response.data?.calculateNernst
                                } catch (e: Exception) {
                                    result = null
                                } finally {
                                    isLoading = false
                        ...
                        enabled = n.isNotBlank() && eo.isNotBlank() && q.isNotBlank()
                    ) {
                        Text("Calculate")
                    }
